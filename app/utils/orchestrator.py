...
def run_experiment_pipeline(cfg: RAGConfig, run_name: str, dataset_type: str):
    ...
    """
    tracker = RAGExperimentTracker(experiment_name="Experimenting hybrid+mmr together")
...
if __name__ == "__main__":
    # Quality-focused sweep: smaller chunks, higher k, MMR, optional reranker
    configs_to_test = [
        # New defaults (smaller chunks, k=16, hybrid + MMR)
        {"chunk_size": 800, "min_chunk_size": 500, "max_chunk_size": 800, "top_k": 16, "use_hybrid": True, "hybrid_weight": 0.6, "use_mmr": True, "use_reranker": False, "temperature": 0.0},
        # Same + reranker for better context precision
        {"chunk_size": 800, "min_chunk_size": 500, "max_chunk_size": 800, "top_k": 16, "use_hybrid": True, "hybrid_weight": 0.6, "use_mmr": True, "use_reranker": True, "rerank_top_n": 8, "temperature": 0.0},
        # Higher k without reranker (more chunks, MMR diversifies)
        {"chunk_size": 800, "min_chunk_size": 500, "max_chunk_size": 800, "top_k": 20, "use_hybrid": True, "hybrid_weight": 0.6, "use_mmr": True, "use_reranker": False, "temperature": 0.0},
    ]
    ...
    for i, params in enumerate(configs_to_test):
        ...
        run_name = f"Exp_{i}_CS{params['chunk_size']}_K{params['top_k']}_{temp_tag}_{model_tag}_{hybrid_tag}_{hw_tag}_{mm_tag}_{mmr_tag}_{rr_tag}"
